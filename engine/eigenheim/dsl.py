...
class DslError(ValueError):
    def __init__(self, msg: str, pos: int | None = None):
        super().__init__(msg if pos is None else f"{msg} (position {pos})")
        self.pos = pos
...
def _eval(node: ast.AST, env: Mapping[str, float]):
    ...
    if isinstance(node, ast.Constant):
        if isinstance(node.value, (int, float)) and not isinstance(node.value, bool):
            return node.value
        raise DslError("only numeric literals are allowed", getattr(node, "col_offset", None))
    if isinstance(node, ast.Name):
        if node.id not in env:
            raise DslError(f"unknown input '{node.id}'", node.col_offset)
        return env[node.id]
    ...
    if isinstance(node, ast.Call):
        if not isinstance(node.func, ast.Name):
            raise DslError("invalid call", node.col_offset)
        fname = node.func.id
        if node.keywords:
            raise DslError("keyword arguments are not supported", node.col_offset)
        if fname == "prev":
            # prev(alias): value of alias over the previous period (for honest deltas)
            if len(node.args) != 1 or not isinstance(node.args[0], ast.Name):
                raise DslError("prev(alias) expects a single input", node.col_offset)
            return env.get("__prev__", {}).get(node.args[0].id)
        if fname not in _FUNCS:
            raise DslError("only ratio/sum/min/max/prev calls are allowed", node.col_offset)
        args = [_eval(a, env) for a in node.args]
        return _FUNCS[fname](*args)
    raise DslError("invalid node in expression", getattr(node, "col_offset", None))
...
def evaluate(expression: str, env: Mapping[str, float], prev_env: Mapping[str, float] | None = None):
    try:
        ...
        raise DslError(f"syntax: {e.msg}", e.offset) from e
    full = dict(env)
